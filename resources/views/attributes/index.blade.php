...
<div class="container">
    ...
    <hr>
	<div class="container">
	<a href="{{ route('attributes.create') }}" class="btn btn-lg btn-info">Nuevo</a>
	</div>
	<hr>
		...
		<table id="table" class="compact hover nowrap row-border">
			...
			@foreach($attributes as $row)
				<tr>
					...
					<td>
						@if($row->trashed())
							<form action="{{ route('attributes.restore', $row->id) }}" method="post" style="display: inline;">
                                {{ method_field('PATCH') }}
                                {{ csrf_field() }}
						...
						<div style="display: inline;">
							<form method="get" action="{{route('attributes.edit', $row)}}" style="display: inline;">
								<button type="submit" class="btn btn-sm btn-primary">Editar</button>
							</form>
							{{-- <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#confirmModal" onclick="setDeleteAction('{{ route('attributes.destroy', $row) }}','{{$row->name}}');">Eliminar</button> --}}
						</div> 
						@endif 
					</td>
				</tr>
